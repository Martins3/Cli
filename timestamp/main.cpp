...
int main(void){
   ...
   syslog(LOG_NOTICE, "TimeStamp:Successfully started");
   ...
   if(sid < 0){
      ...
      syslog(LOG_ERR, "TimeStamp:Could not generate session ID for child process");
   ...
   if((chdir("/")) < 0){
      // Log failure and exit
      syslog(LOG_ERR, "TimeStamp:Could not change working directory to /");
